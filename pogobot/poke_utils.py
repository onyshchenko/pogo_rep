...
def pokemon_iv_percentage(pokemon):
    return round(((pokemon.get('individual_attack', 0) + pokemon.get('individual_stamina', 0) + pokemon.get(
...
def get_inventory_data(res, poke_names):
    ...
    #)
    inventory_items_pokemon_list_sorted = sorted(inventory_items_pokemon_list, key=lambda x: x['pokemon_data']['pokemon_id'])

    poke_list_one = []
    poke_list_new = []
    
    pok_id = 0
    for pokemon in inventory_items_pokemon_list_sorted:
      if pok_id == pokemon['pokemon_data']['pokemon_id']:
        poke_list_one.append(pokemon)
      else:
        if len(poke_list_one) > 0:
          poke_list_one = sorted(poke_list_one, key=lambda x: x['pokemon_data']['cp'], reverse=True)
          for poke_list_one1 in poke_list_one:
            poke_list_new.append(poke_list_one1)
          poke_list_one = []
        
        poke_list_one.append(pokemon)
        pok_id = pokemon['pokemon_data']['pokemon_id']
    ...
    inventory_items_pokemon_list_sorted = poke_list_new
    #print (poke_list_new)
    poke_list_one = []
    poke_list_new = []
    
    #print (inventory_items_pokemon_list_sorted)
    for pokemon in inventory_items_pokemon_list_sorted:
      if pokemon['pokemon_data']['pokemon_id'] == 96:
        print (pokemon)
    	#print ("pokemon_data: ", pokemon['pokemon_data']['pokemon_id'], pokemon['pokemon_data']['cp'], pokemon['pokemon_data']['individual_defense'], pokemon['pokemon_data']['stamina'], pokemon['pokemon_data']['individual_attack']) 
